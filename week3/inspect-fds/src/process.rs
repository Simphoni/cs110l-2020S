...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        let fs_dir = format!("/proc/{}/fd", self.pid);
        let fd_list = fs::read_dir(fs_dir).ok()?;
        let mut ret: Vec<usize> = Vec::new();
        for fd in fd_list {
            let entry = fd.ok()?;
            let rust_str_result = entry.file_name().into_string();
            match rust_str_result {
                Ok(rust_str) => {
                    let fd_num = rust_str.parse::<usize>().ok()?;
                    ret.push(fd_num);
                }
                Err(_os_str) => {
                    print!("Error occurred when converting OsString.");
                    std::process::exit(1);
                }
            }
        }
