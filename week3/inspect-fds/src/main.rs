...
fn main() {
    ...
    let target = &args[1];
    let proc_result = ps_utils::get_target(&target).expect("get_target failed.");
    if proc_result.is_none() {
        println!(
    ...
    let proc = proc_result.unwrap();
    proc.print();
    for child in ps_utils::get_child_processes(proc.pid).expect("get_child_process failed") {
        child.print()
    }
}
